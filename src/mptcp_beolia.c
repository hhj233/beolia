...
// 增加计算带宽评估因子
static u64 mptcp_get_beta(const struct mptcp_cb *mpcb)
{
	struct mptcp_tcp_sock *mptcp;
	...
	mptcp_for_each_sub(mpcb, mptcp) {
		...
		beta += div_u64(mptcp_olia_scale(tmp_int, scale), tmp_rtt);
	}
	return beta;
 } 
...
static void mptcp_olia_cong_avoid(struct sock *sk, u32 ack, u32 acked)
{
	...
	// 求beta---------------------start
	beta = mptcp_get_beta(mpcb); 
	tmp_rtt = tp->srtt_us * tp-> srtt_us;
	tmp_int = max(ca->mptcp_loss3 - ca->mptcp_loss2,
				      ca->mptcp_loss2 - ca->mptcp_loss1);
	beta_up = div_u64(mptcp_olia_scale(tmp_int, scale), tmp_rtt);
	// ---------------------------end
	cwnd_scaled = mptcp_olia_scale(tp->snd_cwnd, scale);
	inc_den = ca->epsilon_den * tp->snd_cwnd * rate * beta ? : 1;

	/* calculate the increasing term, scaling is used to reduce the rounding effect */
	if (ca->epsilon_num == -1) {
		if (ca->epsilon_den * cwnd_scaled * cwnd_scaled < rate*beta_up) {
			inc_num = rate*beta_up - ca->epsilon_den *
				cwnd_scaled * cwnd_scaled;
			ca->mptcp_snd_cwnd_cnt -= div64_u64(
			    mptcp_olia_scale(inc_num , scale) , inc_den);
		} else {
			inc_num = ca->epsilon_den *
			    cwnd_scaled * cwnd_scaled - rate * beta_up;
			ca->mptcp_snd_cwnd_cnt += div64_u64(
			    mptcp_olia_scale(inc_num , scale) , inc_den);
		}
	} else {
		inc_num = ca->epsilon_num * rate * beta_up +
...
static struct tcp_congestion_ops mptcp_beolia = {
...
static int __init mptcp_beolia_register(void)
{
	BUILD_BUG_ON(sizeof(struct mptcp_beolia) > ICSK_CA_PRIV_SIZE);
	return tcp_register_congestion_control(&mptcp_beolia);
}

static void __exit mptcp_beolia_unregister(void)
{
	tcp_unregister_congestion_control(&mptcp_beolia);
}

module_init(mptcp_beolia_register);
module_exit(mptcp_beolia_unregister);
